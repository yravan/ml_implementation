...
def download_imagenet(data_dir: str = './data', variant: str = 'imagenette',
                      size: str = '320') -> str:
    """
    ...
    # =================================================================
        ...
        dataset_dir = data_path / 'imagenet'
...
def _reorganize_val(val_dir: Path, devkit_tar: Optional[Path] = None):
    """
    Reorganize flat val directory into class subdirectories.

    ImageNet val comes as 50,000 images in a flat directory.
    We need them organized as val/n01440764/*.JPEG etc.
    """
    # Standard mapping: val image index -> synset
    # This is the official ILSVRC2012 validation ground truth order
    val_images = sorted(val_dir.glob('*.JPEG'))

    if len(val_images) == 0:
        print("  No images found in val directory to reorganize")
        return

    # Try to get labels from devkit
    synsets = None
    if devkit_tar is not None:
        synsets = _load_val_labels_from_devkit(devkit_tar)
    ...
    if synsets is None:
        ...
        try:
            ...
            print("  Validation set will not be reorganized into class folders.")
            return

    if len(synsets) != len(val_images):
        print(f"  Warning: Label count ({len(synsets)}) != image count ({len(val_images)})")
        return

    # Create class directories and move images
    print(f"  Reorganizing {len(val_images)} val images into class folders...")
    for img_path, synset in zip(val_images, synsets):
        synset = synset.strip()
    ...
    print(f"  Reorganized into {n_classes} class folders")
